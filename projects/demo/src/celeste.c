...
static void PRELUDE_initparticles()
{
    ...
    for (i=0; i<=24; i++)
    {
        ...
        particles[i].s   = 0+P8flr(fp_div(P8rnd(5), 4));
...
{
    P8pal(8,(djump==1 ? 8 : (djump==2 ?(7+P8flr(((int)(fp_div((float)frames, 3.0)))%2)*4) : 12)));
}
...
static void draw_hair(OBJ* obj, int facing)
{
    ...
    {
        h = &obj->hair[i++];
        h->x+=fp_div((last_x-h->x),1.5);
        h->y+=fp_div((last_y+0.5-h->y),1.5);
...
#else
        this->y=fp_mul(this->start+P8sin(this->offset),2);
#endif
...
static void BALLOON_draw(OBJ* this)
{
    if (this->spr==22)
    {
        P8spr(13+(int)(fp_mul(this->offset,8))%3,this->x,this->y+6, 1,1,false,false);
...
static void FRUIT_update(OBJ* this)
{
    ...
    this->off += 1;
    this->y    = this->start+fp_mul(P8sin(fp_div(this->off,40)),2.5f);
}
...
static void FLY_FRUIT_update(OBJ* this)
{
    ...
    else
    {
        ...
        this->step+=0.05;
        this->spd.y=fp_mul(P8sin(this->step),0.5);
    }
    // collect
    hit=OBJ_collide(this, OBJ_PLAYER,0,0);
    if (hit!=NULL)
...
static void FAKE_WALL_update(OBJ* this)
{
    ...
    {
        hit->spd.x     = fp_mul(-sign(hit->spd.x),1.5);
        hit->spd.y     = -1.5;
        hit->dash_time = -1;
        sfx_timer      = 20;
        P8sfx(16);
...
static void KEY_update(OBJ* this)
{
    ...
    this->spr=9+(P8sin(fp_div((float)frames,30.0))+0.5)*1;
    is=P8flr(this->spr);
    if (is==10 && is!=was)
    {
...
{
    this->spd.x=fp_mul(this->dir,0.65);
    if (this->x<-16)
    {
        this->x=128;
    }
...
static void ORB_draw(OBJ* this)
{
    ...
    off=fp_div((float)frames,30.f);
    for (i=0; i <= 7; i+=1)
    {
        P8circfill(this->x+4+fp_mul(P8cos(off+fp_div(i,8.f)),8),this->y+4+fp_mul(P8sin(off+fp_div(i,8.f)),8),1,7);
    }
    if (destroy_self)
    {
        destroy_object(this);
...
{
    this->spr=118+P8modulo((fp_div((float)frames,5.f)), 3);
    P8spr(this->spr,this->x,this->y, 1,1,false,false);
    if (this->show)
    {
...
static void kill_player(OBJ* obj)
{
    ...
    for (dir=0; dir <= 7; dir+=1)
    {
        ...
        dead_particles[dead_particles_count].spd2.x = fp_mul(P8sin(angle),3);
        dead_particles[dead_particles_count].spd2.y = fp_mul(P8cos(angle),3);
...
void Celeste_P8_update()
{
    ...
    redo_update_slot:
        ...
        if (room_just_loaded)
        {
            /*printf("update(): load room (player was: #%i)\n", i),*/
            room_just_loaded = false;
        }
...
void Celeste_P8_draw()
{
    ...
    if (!is_title())
    {
        for (i = 0; i <= 16; i++)
        {
            CLOUD* c = &clouds[i];
            c->x += c->spd;
            P8rectfill(c->x,c->y,c->x+c->w,c->y+4+(1-fp_div(c->w,64.0))*12,new_bg ? 14 : 1);
            if (c->x > 128)
            {
                c->x = -c->w;
                c->y = P8rnd(128-8);
    ...
    for (i = 0; i <= 24; i++)
    {
        ...
        p->x += p->spd;
        p->y += P8sin(p->off);
        p->off+= P8min(0.05,fp_div(p->spd,32));
...
{
    int i;
    for (i=(int)P8max(0,P8flr(fp_div(x,8))); i <= P8min(15,fp_div((x+w-1),8)); i++)
    {
        int j;
        for (j=(int)P8max(0,P8flr(fp_div(y,8))); j <= P8min(15,fp_div((y+h-1),8)); j++)
        {
